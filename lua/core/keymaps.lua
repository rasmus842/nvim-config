...
function NavigateTabBuffers(direction)
	local current_tab = vim.api.nvim_get_current_tabpage()
	local buffers = vim.fn.tabpagebuflist(current_tab) -- Get buffers in the current tab
	local current_buf = vim.api.nvim_get_current_buf()
	local current_index = vim.tbl_indexof(buffers, current_buf)
	...
	local next_index
	if direction == "n" then
		next_index = (current_index % #buffers) + 1
	elseif direction == "p" then
		next_index = (current_index - 2) % #buffers + 1
	end
...
-- Helper to find index of value in table
